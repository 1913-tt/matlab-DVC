...
%通过强制复子带为解析的(在二维傅里叶平面的一半上为零，除非它们被修改，否则它们就是对偶的)，
%并由解析子带的实部重建图像。这相当于计算子带的虚部的希尔伯特变换，将它们与实部平均，然后从得到的实部带重新构造
...
for nsc = 1:Nsc,
    ...
    for nor = 1:Nor,
        ...
        f = 1;
        ch = f*0.5*real(ch); % real part 。     这里么什么要x0．5和f  因为amask乘了个2？
        %pyr(ind) = ch;
        pyrTmp = [pyrTmp; ch(:)];
    end     % nor
end         % nsc
