...
%  log_rad = log_rad + 1;
  ...
  const = (2^(2*order))*(factorial(order)^2)/(nbands*factorial(2*order));%factorial计算阶层
...
%
  ...
  for b = 1:nbands
    anglemask = pointOp(angleI, Ycosn, Xcosn(1)+pi*(b-1)/nbands, Xcosn(2)-Xcosn(1));%%选择角度？？
...
%     end
  ...
  lodims = round(im_dims(1:2)*scale^(nScales-ht+1)); %计算下一次的大小
  
  loctr = ceil((lodims+0.5)/2);
  lostart = ctr-loctr+1;
  loend = lostart+lodims-1;

  log_rad = log_rad(lostart(1):loend(1),lostart(2):loend(2));%怎么直接就把边边减掉了
